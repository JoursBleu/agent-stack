...
agent-stack/
  ...
  seeds/                     用户首启时拷贝到 per-user 家目录的种子配置
    ...
    nanobot-home/config.json
  images/openclaw/           可选的 OpenClaw + Chromium 薄镜像层
  images/nanobot/            HKUDS nanobot Dockerfile（本地 build）
...
# 3. 拉后端镜像。docker compose 只 build router/frontend，**不会**
#    自动拉后端镜像，第一次必须手动拉。
docker pull ghcr.io/openclaw/openclaw:latest
docker pull nousresearch/hermes-agent:latest
# 或 build 本仓库带桌面 Chromium 的加层：
#   cd images/openclaw && docker build -t openclaw-with-chromium:latest .
# nanobot 没有发布镜像，需要本地 build（说明见 docs/nanobot-integration.md）：
#   cd images/nanobot && docker build -t agstack/nanobot-hkuds:0.1.5.post3 .
